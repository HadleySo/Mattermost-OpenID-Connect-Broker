...
def get_jwt_config():
    ...
    return {
        ...
        'alg': current_app.config.get('OAUTH2_JWT_ALG', 'RS256'),
        'iss': current_app.config.get('OAUTH2_JWT_ISS', current_app.config['ife_IDP_DOMAIN']),
        'exp': current_app.config.get('OAUTH2_JWT_EXP', 3600),
    }


def generate_user_info(user, scope):
    return UserInfo(sub=str(user.mattermost_id))

def user_info_api(user, scope):
    from ..appActions import user_manager
    user_data = user_manager.get_oidc(user.mattermost_id)
